...
exports.UserAccessReviewer = void 0;
const core_1 = require("@sap-framework/core");
const sodRules_1 = require("./rules/sodRules");
class UserAccessReviewer {
    ipsConnector;
    ruleEngine;
    config;
    constructor(ipsConnector, config = {}) {
        this.ipsConnector = ipsConnector;
        this.ruleEngine = new core_1.RuleEngine();
        this.config = {
            dataSource: {
                type: 'IPS',
                userIdField: 'userId',
    ...
    async fetchUserAccess() {
        ...
        const BATCH_SIZE = 50;
        const usersToProcess = ipsUsers.filter(user => this.config.analysis.includeInactiveUsers || user.active);
        for (let i = 0; i < usersToProcess.length; i += BATCH_SIZE) {
            const batch = usersToProcess.slice(i, i + BATCH_SIZE);
            // Process batch in parallel
            const batchResults = await Promise.all(batch.map(async (ipsUser) => {
    ...
    convertSoDRulesToFrameworkRules() {
        const sodRules = [
            ...sodRules_1.STANDARD_SOD_RULES,
            ...(this.config.analysis.customRules || [])
        ];
        const filteredRules = sodRules.filter(rule => rule.riskScore >= this.config.analysis.minimumRiskScore);
    ...
    convertToSoDViolations(frameworkViolations) {
    ...
    generateAnalysisResult(users, violations) {
    ...
    generateRecommendations(summary, _violations) {
        const recommendations = [];
        if (summary.criticalViolations > 0) {
            recommendations.push(`🔴 CRITICAL: ${summary.criticalViolations} critical violations detected. Immediate remediation required.`);
        }
        if (summary.highViolations > 10) {
            recommendations.push(`⚠️  HIGH: ${summary.highViolations} high-risk violations found. Implement mitigation controls.`);
        }
        const violationRate = (summary.usersWithViolations / summary.totalUsers) * 100;
        if (violationRate > 10) {
            recommendations.push(`${violationRate.toFixed(1)}% of users have SoD violations. Consider role redesign.`);
        }
        return recommendations;
    }
    getSoDRuleById(ruleId) {
        return [...sodRules_1.STANDARD_SOD_RULES, ...(this.config.analysis.customRules || [])]
    ...
    getStats() {
        return {
            ...
            version: '1.0.0',
            rulesLoaded: sodRules_1.STANDARD_SOD_RULES.length + (this.config.analysis.customRules?.length || 0),
            configuration: this.config,
        };
    }
}
exports.UserAccessReviewer = UserAccessReviewer;
//# sourceMappingURL=UserAccessReviewer.js.map