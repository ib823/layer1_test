...
exports.validateDatabaseEncryption = validateDatabaseEncryption;
exports.enforceEncryptionRequirement = enforceEncryptionRequirement;
exports.getComplianceEncryptionRequirements = getComplianceEncryptionRequirements;
const pg_1 = require("pg");
/**
 * Validate PostgreSQL database encryption configuration
 */
async function validateDatabaseEncryption(connectionString) {
    ...
    `);
        if (sslResult.rows.length > 0 && sslResult.rows[0].ssl) {
            ...
            status.recommendations.push('Enable SSL/TLS by configuring sslmode=require in connection string');
        }
        ...
        const isCloudDatabase = cloudIndicators.some((indicator) => connectionString.includes(indicator));
        if (isCloudDatabase) {
            status.enabled = true;
            status.method = 'Cloud Provider Managed Encryption';
            status.details.transparentDataEncryption = true;
            status.recommendations.push('Verify encryption at rest is enabled in your cloud provider console');
        }
    ...
    `);
        if (extensionsResult.rows.some((row) => row.extname === 'pg_tde')) {
            ...
            status.warnings.push('pgcrypto extension is available but not the same as full disk encryption');
        ...
        try {
            ...
            status.recommendations.push('Ensure filesystem/disk encryption (LUKS, dm-crypt) is enabled at OS level');
        }
        catch (error) {
        ...
        if (!status.enabled) {
            ...
            status.recommendations.push('Enable encryption at rest using one of the following methods:', '1. Use managed database service with encryption (AWS RDS, Azure Database, etc.)', '2. Enable filesystem encryption (LUKS, dm-crypt)', '3. Use PostgreSQL TDE extension', '4. Configure SSL/TLS for data in transit');
        }
        // SSL is minimum requirement
        if (!status.details.sslEnabled) {
            status.recommendations.push('CRITICAL: Enable SSL/TLS for database connections immediately');
        }
        return status;
    }
    catch (error) {
 ...
 */
async function enforceEncryptionRequirement(connectionString, required = true) {
    if (!required) {
 ...
 */
function getComplianceEncryptionRequirements(frameworks) {
    const requirements = [];
...
//# sourceMappingURL=dbEncryptionValidator.js.map