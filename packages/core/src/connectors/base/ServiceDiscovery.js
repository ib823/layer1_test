...
class ServiceDiscovery {
    ...
    async discoverServices() {
        ...
        try {
            ...
            const catalogResponse = await this.connector.executeRequest({
                method: 'GET',
     ...
     */
    async getServiceMetadata(serviceUrl) {
        try {
            const metadataUrl = `${serviceUrl}/$metadata`;
            const metadataXML = await this.connector.executeRequest({
                method: 'GET',
                url: metadataUrl,
            });
     ...
     */
    async testServicePermissions(services) {
        const results = [];
     ...
     */
    assessCapabilities(services, permissionTests) {
        const hasService = (technicalName) => {
            return (services.some(s => s.technicalName === technicalName && s.status === 'ACTIVE') &&
                permissionTests.some(p => p.service === technicalName && p.accessible));
        };
        return {
            canDoSoD: hasService('API_USER_SRV') && hasService('API_ROLE_SRV'),
            canDoInvoiceMatching: hasService('API_PURCHASEORDER_SRV') && hasService('API_INVOICE_SRV'),
     ...
     */
    async detectSAPVersion() {
        try {
            // Query system information
            const response = await this.connector.executeRequest({
                method: 'GET',
     ...
     */
    async generateTenantProfile(tenantId) {
     ...
     */
    identifyMissingServices(services) {
     ...
     */
    generateRecommendedActions(discovery) {
        const actions = [];
    ...
    inferRequiredAuths(serviceName) {
    ...
    mapVersion(version) {
        ...
        if (version.includes('ECC'))
            return 'ECC6';
        return 'UNKNOWN';
    }
    getPriority(service) {
        if (service.includes('USER') || service.includes('ROLE'))
            return 'HIGH';
        if (service.includes('PURCHASEORDER') || service.includes('GLACCOUNT'))
            return 'MEDIUM';
        return 'LOW';
    }
    getReason(service) {
    ...
    getActivationInstructions(service) {
        return [
